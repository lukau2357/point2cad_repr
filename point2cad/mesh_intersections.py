...
def compute_vertices_from_point_matching(polyline_map, match_threshold=1e-3,
                                          cluster_radius=5e-3,
                                          tangent_edges=None,
                                          tangent_threshold=1e-2):
    """
    Find B-Rep vertices by matching polyline points between edge pairs.

    For each pair of edges (i,j) and (j,k) sharing exactly one face,
    find points in one polyline that are very close to points in the other.
    These shared points correspond to mesh intersection vertices where
    three surfaces meet.

    This is more robust than segment-segment crossing because it matches
    actual mesh intersection points rather than looking for geometric
    crossings between interpolated segments.

    Parameters
    ----------
    polyline_map       : dict (i, j) -> list of (N, 3) arrays
    match_threshold    : max distance between two polyline points to
                         consider them a match (should be small — mesh precision)
    cluster_radius     : greedy deduplication radius for final vertices
    tangent_edges      : set of edge keys generated by tangent fallback
    tangent_threshold  : wider match threshold for tangent edges
    ...
    for a_idx in range(len(edge_keys)):
        for b_idx in range(a_idx + 1, len(edge_keys)):
            ...
            n_pairs_tested += 1
            # Use wider threshold if either edge is tangent-generated
            thr = (tangent_threshold
                   if edge_a in tangent_edges or edge_b in tangent_edges
            ...
            for pa_idx, poly_a in enumerate(polyline_map[edge_a]):
                ...
                for pb_idx, poly_b in enumerate(polyline_map[edge_b]):
                    ...
                    for ai, (dist, bi) in enumerate(zip(dists, indices)):
                        if dist < thr:
                            pos = 0.5 * (poly_a[ai] + poly_b[bi])
                            candidates.append((
                                pos, dist,
                                edge_a, pa_idx, edge_b, pb_idx,
                            ))

    print(f"[point-match] {n_pairs_tested} edge pairs tested")
    print(f"[point-match] {len(candidates)} raw candidates "
          f"(match_threshold={match_threshold:.1e})")
    ...
    # Greedy clustering (same as segment intersection method)
    positions = np.array([c[0] for c in candidates], dtype=np.float64)
    used = np.zeros(len(positions), dtype=bool)
    vertices = []
    vertex_edges = []
    ...
    print(f"[point-match] {len(vertices)} vertices after clustering")

    # Post-clustering attribution (same as segment intersection method)
    from itertools import combinations
    for v_idx in range(len(vertices)):
        pos = vertices[v_idx]
        incident_faces = set()
...
def merge_nearby_vertices(vertices, vertex_edges, vertex_polys,
                          merge_radius=1.5e-2):
    """
    Second-pass merging: collapse nearby vertices that share ≥1 common edge.

    Mesh interleaving near curved surfaces can produce clusters of spurious
    vertices around a single physical corner.  This pass merges them using
    union-find with two conditions:
      1. Spatial proximity (within merge_radius)
      2. Topological overlap (at least one shared edge key)

    The merged vertex inherits the union of all edge sets and poly sets,
    positioned at the centroid of the cluster.

    Returns (vertices, vertex_edges, vertex_polys) with reduced vertex count.
    """
    n = len(vertices)
    if n == 0:
        return vertices, vertex_edges, vertex_polys
    ...
    for i in range(n):
        neighbors = tree.query_ball_point(vertices[i], merge_radius)
        for j in neighbors:
            if j <= i:
                continue
            if find(i) == find(j):
            ...
            if vertex_edges[i] & vertex_edges[j]:
                union(i, j)
    ...
    for root, members in sorted(groups.items()):
        ...
        pos = np.mean([vertices[m] for m in members], axis=0)
        edges = set()
        polys = set()
        for m in members:
            edges |= vertex_edges[m]
            polys |= vertex_polys[m]
        ...
        if len(members) > 1:
            merged_count += 1
            print(f"  [merge] v{new_idx}: merged {sorted(members)} "
                  f"({len(members)} vertices)  "
                  f"edges={sorted(edges)}")

    # Remap poly references: (edge_key, poly_idx) doesn't reference vertex
    # indices, so no remapping needed there.  But vertex_polys sets are
    # already unioned above.

    print(f"[vertex-merge] {n} → {len(new_vertices)} vertices "
          f"({merged_count} groups merged)")

    verts_arr = (np.array(new_vertices, dtype=np.float64) if new_vertices
                 else np.empty((0, 3), dtype=np.float64))
    return verts_arr, new_edges, new_polys, old_to_new
